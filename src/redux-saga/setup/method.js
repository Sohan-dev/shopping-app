...
export async function PATCH(URL, PAYLOAD, TOKEN, HEADER = json_data) {
  try {
    let response = await patchApi(URL, PAYLOAD, TOKEN, HEADER);
...
export async function DELETE(URL, TOKEN) {
  try {
    console.log(TOKEN);
    let response = await deleteApi(URL, TOKEN);
